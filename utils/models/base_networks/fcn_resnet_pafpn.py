...
class FCNPaFPN(nn.Module):
    def __init__(self, n_classes):
        ...
        self.score_32s_n5 = nn.Conv2d(256, self.n_classes, kernel_size=1)
        self.score_16s_p4 = nn.Conv2d(256, self.n_classes, kernel_size=1)
        self.score_16s_n4 = nn.Conv2d(256, self.n_classes, kernel_size=1)
        self.score_8s_p3 = nn.Conv2d(256, self.n_classes, kernel_size=1)
        self.score_8s_n3 = nn.Conv2d(256, self.n_classes, kernel_size=1)
        self.score_4s = nn.Conv2d(256, self.n_classes, kernel_size=1)

        self.fpn = PAFPN([256, 512, 1024, 2048], 256)
    ...
    def forward(self, x):
        ...
        p2, p3, p4, p5, n3, n4, n5 = self.fpn(c2, c3, c4, c5)

        res_32s = self.score_32s_p5(p5) + self.score_32s_n5(n5)
        res_16s = self.score_16s_p4(p4) + self.score_16s_n4(n4) + nn.functional.interpolate(res_32s, scale_factor=2, mode='bilinear', align_corners=True)
        res_8s = self.score_8s_p3(p3) + self.score_8s_n3(n3) + nn.functional.interpolate(res_16s, scale_factor=2, mode='bilinear', align_corners=True)
