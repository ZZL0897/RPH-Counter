...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    filters_per_channel = out_channels // groups
    conv_per_position_flops = np.prod(kernel_dims, dtype=np.int64) * in_channels * filters_per_channel

    active_elements_count = batch_size * np.prod(output_dims, dtype=np.int64)
